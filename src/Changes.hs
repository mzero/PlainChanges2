...
{-
    ...
    first change (which, if descending pitches, is indeed treble and tenor).
-}

-- | Generate all permutations of the supplied sequence. The permutations are
-- produced by the method known as Plain Changes. The supplied sequence will
-- also be the first permutation.
--
-- Note: The returned permutations do not include a repeate of the initial
-- permutation at the end, even thought that is how changes are commonly played.
ringOf :: [a] -> [[a]]
ringOf [] = [[]]
ringOf [a] = [[a]]
