...
-- Low and high keys are inclusive.
...
-- | Channels assigned to the strings. In same order as `bassStrings`.
...
-- | Return the fret position to play a note on a string, if the pitch can be
-- played by the given string.
...
-- These times were as reported by MechBass creators in their paper. They
-- reflect the timing of an earlier firmware.
...
data StringState
    ...
    | Damped Fret               -- not sounding, shifter at fret
...
data MechBassError
    ...
    | BadNoteOff
    | KeyOutOfRange Key
    | StartsLate Time   -- not currently used
    | FretTooLate Time  -- not currently used
  deriving (Eq, Show)
